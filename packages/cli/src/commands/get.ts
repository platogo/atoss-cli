...
import type { DailyTimeData } from '@atoss/shared';
...
export async function get(dateString?: string): Promise<void> {
  const targetDate = dateString ? parseDate(dateString) : new Date();
  const formattedDate = formatDate(targetDate);

  console.log(`Fetching time tracking data for ${formattedDate}...`);
  ...
  try {
    ...
    const data = response.data as DailyTimeData;

    // Display results
    console.log('\n✓ Time tracking data retrieved:');
